...
// ---------------------------------------------------------------------------
// GET /api/reports/summary  — school-wide dashboard summary
// FIX: was 5 sequential queries — now 1 after term is resolved
// ---------------------------------------------------------------------------
router.get("/summary", async (req, res) => {
  try {
    const result = await db.query(
      `WITH active_term AS (
       ...
       per_student AS (
         -- One row per active student: what they owe vs what they paid
         SELECT
           s.id                                       AS student_id,
           COALESCE(sf.amount, 0)                     AS fee,
           COALESCE(SUM(p.amount_paid), 0)            AS paid
         FROM students s
         JOIN classes c     ON s.class_id = c.id
         LEFT JOIN school_fees sf
           ON sf.class_id = c.id
          AND sf.term_id  = (SELECT id FROM active_term)
         LEFT JOIN payments p
           ON p.student_id = s.id
          AND p.term_id    = (SELECT id FROM active_term)
         WHERE s.status = 'Active'
         GROUP BY s.id, sf.amount
       )
       SELECT
         (SELECT id   FROM active_term)                                   AS term_id,
         COUNT(*)                                                         AS total_students,
         COUNT(*) FILTER (WHERE paid >= fee AND fee > 0)                  AS fully_paid,
         COUNT(*) FILTER (WHERE paid < fee  OR fee = 0)                   AS owing,
         SUM(paid)                                                        AS total_amount,
         SUM(fee)                                                         AS total_expected_fee
       FROM per_student`
    );
    ...
    res.json({
      ...
      owing:             parseInt(row.owing,             10),
      total_amount:      parseFloat(row.total_amount     || 0),
      total_expected_fee:parseFloat(row.total_expected_fee || 0),
    });
  } catch (err) {
    console.error("Dashboard summary error:", err);
    res.status(500).json({ error: err.message });
...
// ---------------------------------------------------------------------------
// GET /api/reports/summary/:section  — section-scoped summary
// FIX: was 5 sequential queries — now 1 after term is resolved
// ---------------------------------------------------------------------------
router.get("/summary/:section", async (req, res) => {
  try {
...
// FIX: correlated subquery ran once per row — replaced with a window function
// ---------------------------------------------------------------------------
router.get("/recent-payments", async (req, res) => {
  try {
    const result = await db.query(
      `WITH active_term AS (
       ...
       running_totals AS (
         -- Pre-aggregate total paid per student so no correlated subquery needed
         SELECT
           student_id,
           SUM(amount_paid) AS total_paid_by_student
         FROM payments
       ...
       SELECT
         ...
         COALESCE(sf.amount, 0) - COALESCE(rt.total_paid_by_student, 0) AS balance
       FROM payments p
       JOIN students s      ON p.student_id = s.id
       JOIN classes c       ON s.class_id   = c.id
       ...
       LEFT JOIN running_totals rt
         ON rt.student_id = s.id
       WHERE s.status = 'Active'
         AND p.term_id  = at.id
